...
fn cmp_random(c: &mut Criterion) {
    ...
    for len in INPUT_LENGTHS {
        ...
        group.bench_with_input(BenchmarkId::new("RcString", len), &len, |b, &len| {
            b.iter_batched_ref(
                || {
                    (
                        RcString::try_from(random_string(len).as_str()).unwrap(),
                        RcString::try_from(random_string(len).as_str()).unwrap(),
                    )
                },
                |(a, b)| a.cmp(&b),
        ...
        group.bench_with_input(BenchmarkId::new("RcString", len), &len, |b, &len| {
            b.iter_batched_ref(
                || {
                    let s = random_string(len);
                    (
                        RcString::try_from(s.as_str()).unwrap(),
                        RcString::try_from(s.as_str()).unwrap(),
                    )
                },
                |(a, b)| a.cmp(&b),
    ...
    for len in INPUT_LENGTHS {
        ...
        group.bench_with_input(BenchmarkId::new("RcString", len), &len, |b, &len| {
            b.iter_batched_ref(
                || {
                    let s = random_string(len);
                    (RcString::try_from(s.as_str()).unwrap(), s)
                },
                |(a, b)| PartialEq::eq(a, b),
                criterion::BatchSize::SmallInput,
            )
...
fn construct_empty(c: &mut Criterion) {
    ...
    group.bench_function(BenchmarkId::new("RcString", "copy"), |b| {
        b.iter_batched(
            String::new,
            |s| RcString::try_from(s.as_str()),
            criterion::BatchSize::SmallInput,
        )
    });
    group.bench_function(BenchmarkId::new("RcString", "move"), |b| {
        b.iter_batched(
            String::new,
            RcString::try_from,
            criterion::BatchSize::SmallInput,
...
fn construct_non_empty(c: &mut Criterion) {
    ...
    for len in INPUT_LENGTHS {
        ...
        group.bench_with_input(BenchmarkId::new("RcStringCopy", len), &len, |b, &len| {
            b.iter_batched(
                || random_string(len),
                |s| RcString::try_from(s.as_str()),
                criterion::BatchSize::SmallInput,
        ...
        group.bench_with_input(BenchmarkId::new("RcStringMove", len), &len, |b, &len| {
            b.iter_batched(
                || random_string(len),
                RcString::try_from,
                criterion::BatchSize::SmallInput,
